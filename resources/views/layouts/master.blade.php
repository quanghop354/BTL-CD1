        ...
        <div class="sidebar bg-dark text-white p-3 d-flex flex-column" style="width: 260px; flex-shrink: 0; box-shadow: 2px 0 10px rgba(0,0,0,0.2); z-index: 10;">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                @auth
                    ...
                    @if(Auth::user()->isUser())
                    <li class="nav-item mb-2">
                        <a href="{{ route('carts.index') }}" class="nav-link text-white">
                    ...
                    @if(Auth::user()->isAdmin() || Auth::user()->isStaff())
                        ...
                        <li class="nav-item mb-2">
                            <a href="{{ route('admin.carts.index') }}" class="nav-link text-white">
                                <i class="fas fa-shopping-basket me-2"></i> Quản Lý Giỏ Hàng
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a href="{{ route('reports.revenue') }}" class="nav-link text-white">
                    ...
                    <li class="nav-item mb-2">
                        <a href="{{ route('payments.index') }}" class="nav-link text-white">
                            <i class="fas fa-file-invoice-dollar me-2"></i> {{ (Auth::user()->isAdmin() || Auth::user()->isStaff()) ? 'Thanh Toán' : 'Lịch Sử Giao Dịch' }}
                        </a>
                    </li>
                @else
                    <li class="nav-item mb-2">
            ...
            <div class="dropdown">
                <a href="#" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle" id="dropdownUser1" data-bs-toggle="dropdown" aria-expanded="false">
                    @if(Auth::user()->avatar)
                        <img src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="{{ Auth::user()->name }}" class="rounded-circle me-2" width="32" height="32" style="object-fit: cover;">
                    @else
                        <i class="fas fa-user-circle fa-2x me-2"></i>
                    @endif
                    <strong class="me-2">{{ Auth::user()->name }}</strong>
                    <span class="badge bg-info ms-auto">{{ Auth::user()->isAdmin() ? 'Admin' : (Auth::user()->isStaff() ? 'Staff' : 'User') }}</span>
                </a>
