...
Route::middleware('auth')->group(function () {
    ...
    // POST: Xem số điện thoại
    Route::post('/profile/reveal-phone', [AuthController::class, 'revealPhone'])->name('profile.reveal-phone');
    ...
    Route::get('publishers/{publisher}', [\App\Http\Controllers\PublisherController::class, 'show'])->name('publishers.show')->whereNumber('publisher');
    ...
    /**
     * ========================================
     * ROUTES GIỎ HÀNG - TẤT CẢ NGƯỜI DÙNG
     * ========================================
     */
    ...
    Route::put('carts/{cart}', [\App\Http\Controllers\CartController::class, 'update'])->name('carts.update');
    Route::delete('carts/{cart}', [\App\Http\Controllers\CartController::class, 'destroy'])->name('carts.destroy');

    /**
     * ========================================
     * ROUTES QUẢN TRỊ - ADMIN & STAFF
     * ========================================
     * 
     * Middleware 'staff': Kiểm tra xem người dùng có vai trò 'admin' hoặc 'staff' không
     * Nếu không → Lỗi 403 Forbidden
     * 
     * Bao gồm:
     * - Tạo/Sửa/Xóa Sách
    ...
    Route::middleware('staff')->group(function () {
        ...
        // Quản lý giỏ hàng của tất cả user
        Route::get('manage-carts', [\App\Http\Controllers\CartController::class, 'adminIndex'])->name('admin.carts.index');

        // Báo cáo doanh thu
        Route::get('reports/revenue', [\App\Http\Controllers\ReportController::class, 'revenue'])->name('reports.revenue');
        Route::get('reports/revenue/export', [\App\Http\Controllers\ReportController::class, 'exportExcel'])->name('reports.revenue.export');
    });
});
